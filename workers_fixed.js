export default {
  async fetch(request, env) {
    ...
    // ADMIN_PATH 允许用户在环境变量里配置（例如 "admin"）。
    // 做兜底与清洗：避免未配置时变成 "/undefined"，以及用户误填前后斜杠导致路由不匹配。
    ...
    switch (pathname) {
      ...
      case '/api/albums/images':
        return request.method === 'GET'
          ...
          : new Response('Method Not Allowed', { status: 405 });
      case '/albums':
        return await handleAlbumsPage(request, DATABASE, USERNAME, PASSWORD, adminPath);
      // ==================== 相册管理路由结束 ====================
...
// 相册管理页面
async function handleAlbumsPage(request, DATABASE, USERNAME, PASSWORD, adminPath) {
  if (!authenticate(request, USERNAME, PASSWORD)) {
