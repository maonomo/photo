export default {
  async fetch(request, env) {
    ...
    switch (pathname) {
      ...
      default:
        return await handleImageRequest(request, env, DATABASE, R2_BUCKET);
    }
  }
};
...
function isHotlinkAllowed(request, env) {
  const enabled = (env.HOTLINK_PROTECT === 'true');
  if (!enabled) return true;

  // 允许列表：逗号分隔域名（host），例如：example.com,www.example.com,telegra.ph
  const allowlist = (env.HOTLINK_ALLOWLIST || '')
    .split(',')
  ...
  // Referer/Origin 为空时是否放行（兼容“直接打开图片链接”、部分 App/隐私设置）
  ...
  const refHost = pickHost(referer);
  const orgHost = pickHost(origin);

  if (!refHost && !orgHost) return ALLOW_EMPTY;

  if (refHost && allowlist.includes(refHost)) return true;
  if (orgHost && allowlist.includes(orgHost)) return true;
...
<body>
  ...
  <div class="card">
      ...
      $(document).ready(function() {
        ...
        async function uploadFile(file, fileHash) {
          try {
            ...
            if (file.type.startsWith('image/') && file.type !== 'image/gif') {
              // 统一转成 WebP；压缩开关只控制是否缩放（关闭时保留原尺寸）
              toastr.info('正在处理...', '', { timeOut: 0 });
              const processedFile = await compressImage(file, 0.85, interfaceInfo.enableCompression ? 3000 : null);
              file = processedFile;
            }
        ...
        async function compressImage(file, quality = 0.85, maxSide = 3000) {
          const MAX_SIZE = maxSide; // 最大边长（null 表示不缩放）
          ...
          try {
            ...
            toastr.success('图片已处理为' + targetWidth + 'x' + targetHeight + '（WebP / 质量' + Math.round(quality * 100) + '%）');
...
async function handleImageRequest(request, env, DATABASE, R2_BUCKET) {
  // 防盗链：不通过直接拒绝
  if (!isHotlinkAllowed(request, env)) {
    return new Response('Hotlink forbidden', { status: 403 });
  }

  const requestedUrl = request.url;

  // ⚠️ 重要：开启防盗链时，建议跳过 caches.default，避免缓存被“允许来源”写入后被盗链复用
  const hotlinkEnabled = (env.HOTLINK_PROTECT === 'true');

  let cache, cacheKey, cachedResponse;
  if (!hotlinkEnabled) {
    cache = caches.default;
    cacheKey = new Request(requestedUrl);
